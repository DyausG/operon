...
class DemoScenarioRunner:
    ...
    async def approve(self, equipment_id: str, command: dict | None) -> dict:
        ...
        view["approval_decisions"].append(self._record(
            "DEMO-APPROVAL-01", decision="APPROVE", actor_id=command.get("actor_id", "dashboard-operator"),
    ...
    async def _run(self, generation: int) -> None:
        try:
            ...
            self._assert_current(generation)
            view = self._view()
            view["evidence"].extend(self._baseline_evidence())
            ...
            self._assert_current(generation)
            view = self._view()
            view["evidence"].append(self._inspection_evidence())
            ...
            view["verdicts"].append(self._record("DEMO-VERDICT-DIAGNOSIS", target_kind="diagnosis",
                target_id="DEMO-DIAGNOSIS-01", decision="ACCEPT", blocking_issues=[],
            ...
            await self._wait(4)
            self._assert_current(generation)
            view["verdicts"].append(self._record("DEMO-VERDICT-INTERVENTION", target_kind="intervention",
                target_id="DEMO-INTERVENTION-01", decision="ACCEPT", blocking_issues=[],
    ...
    async def _run_after_approval(self, generation: int) -> None:
        try:
            ...
            await self._wait(4)
            self._assert_current(generation)
            view["observation_plans"] = [self._record("DEMO-OBSERVATION-01", status="ACTIVE",
            ...
            for index, (delay, sample) in enumerate(zip((4, 4, 4, 2), RECOVERY), start=1):
                ...
                observation = self._record(f"DEMO-RECOVERY-{index:02d}", sequence=index,
                    failure_risk=risk, health_score=health, vibration_mm_s=metrics["vibration"],
                    temperature_rise_c=metrics["temp_diff"], status="HEALTHY" if risk < .40 else "IMPROVING")
            ...
            await self._wait(2)
            self._assert_current(generation)
            view["outcomes"] = [self._record("DEMO-OUTCOME-01", result="VERIFIED_RECOVERY",
                reason="Four scripted post-intervention observations show sustained risk and vibration recovery.",
    ...
    def _create_incident(self) -> None:
        ...
        signal = self._record("DEMO-EVIDENCE-SIGNAL", kind="model_signal",
            summary="Predictive risk rose from 0.07 to 0.86 and crossed the scripted 0.80 threshold.",
        ...
        lifecycle = {"phase": "OPEN", "revision": 1, "context_revision": 1,
            ...
            "runtime": RUNTIME, "live_model": False, "read_model": {"phase": "OPEN",
                ...
                "approval_decisions": [], "execution_receipts": [], "observation_plans": [], "outcomes": []}}
    ...
    def _event(self, event_type: str, payload: dict) -> None:
        ...
        view["events"].append(self._record(f"DEMO-EVENT-{len(view['events']) + 1:02d}",
                                           event_type=event_type, revision=revision, payload=payload))
    ...
    def _baseline_evidence(self) -> list[dict]:
        return [
            self._record("DEMO-EVIDENCE-TREND", kind="telemetry",
                summary="Drive-end vibration climbed 1.5 → 9.2 mm/s as temperature rise reached 14.8 °C.",
            ...
            self._record("DEMO-EVIDENCE-OPERATING", kind="operational_context",
                summary="Compressor held 78% load; no process upset or commanded speed change explains the trend.",
            ...
            self._record("DEMO-EVIDENCE-HISTORY", kind="maintenance_history",
                summary="Last bearing service was 4,180 operating hours ago; alignment check was previously satisfactory.",
    ...
    def _inspection_evidence(self) -> dict:
        return self._record("DEMO-EVIDENCE-INSPECTION", kind="inspection",
    ...
    def _diagnosis(self) -> dict:
        return self._record("DEMO-DIAGNOSIS-01",
    ...
    def _agent_run(self, stage: str, status: str, specialists: list[tuple[str, str]]) -> dict:
        return self._record(f"DEMO-RUN-{len(self._view().get('agent_runs', [])) + 1:02d}", run_id=f"DEMO-RUN-{stage}",
            stage=stage, status=status, disposition="SIMULATED_ADVISORY", tool_calls=len(specialists),
            summary="Scripted specialist outputs; no live Bedrock, AgentCore, or Strands execution.",
            runtime_identity={"backend": "scripted-demo", "runtime": RUNTIME, "provenance": PROVENANCE,
                              "live_model": False}, blockers=[], assessments=[],
            delegations=[{"key": f"demo-{role}", "role": role, "question": text, "summary": text,
                          "status": "SUCCEEDED", "provenance": PROVENANCE, "runtime": RUNTIME,
                          "live_model": False} for role, text in specialists])

    def _intervention(self) -> dict:
        return self._record("DEMO-INTERVENTION-01", status="DRAFT", risk="MEDIUM", priority="HIGH",
    ...
    def _binding(self) -> dict:
        return {"technician_id": "TECH-DEMO-03", "qualification": "Rotating equipment mechanical maintenance",
    ...
    def _execution_receipt(self) -> dict:
        return self._record("DEMO-RECEIPT-01", receipt_id="DEMO-RECEIPT-01", status="SUCCEEDED",
    ...
    def _requirement(self) -> dict:
        return self._record("DEMO-REQUIREMENT-01", requirement_id="DEMO-REQUIREMENT-01",
            intervention_id="DEMO-INTERVENTION-01", intervention_hash=self._intervention_hash(),
            status="PENDING", policy_version="operon.demo.human-gate-v1", minimum_distinct_approvers=1,
