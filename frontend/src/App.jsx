...
function Header({ state, onReset, onStop, onResume, onDemo }) {
  ...
  return <header className="topbar">
    ...
    {state.demoScenario.active ? <div className="provenance"><span className="hot">Guided Demo · Simulated Plant</span><i>·</i><span>SIMULATED</span><i>·</i><span>no live model</span><em className="standby">{state.demoScenario.status?.replaceAll("_", " ")}</em></div>
    ...
    <span className={`stream ${state.connected && state.running ? "online" : "standby"}`}><i />{!state.connected ? "reconnecting" : state.demoScenario.active ? "SIMULATED" : state.running ? "live" : "paused"}</span>
    <span className="plant-clock">+{String(hours).padStart(2, "0")}:{String(mins).padStart(2, "0")}</span>
    {!state.demoScenario.active && <button className="icon-btn" onClick={state.running ? onStop : onResume}>{state.running ? "Ⅱ" : "▶"}</button>}
...
function AssetCard({ asset, selected, onClick }) {
  return <button className={`asset-card ${asset.status.toLowerCase()} ${selected ? "selected" : ""}`} onClick={onClick}>
    ...
    <div className="risk-row"><b>{pct(asset.failure_prob)}</b><span>24h risk</span></div><div className="asset-mode">{asset.status_source === "active_incident" ? asset.status_reason : asset.predicted_mode && asset.predicted_mode !== "NONE" ? asset.predicted_mode_label : "Nominal signature"}</div><Spark point={asset.point} status={asset.status} />
  </button>;
}
...
function IncidentCommand({ incident, state, approve, reject }) {
  ...
  return <motion.div key={incident.incident_id} className="incident-command" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <Lifecycle phase={phase} events={view.events || []} simulated={state.demoScenario.active} /><Invariant />
...
function Lifecycle({ phase, events, simulated = false }) {
  ...
  return <section className="lifecycle-block"><div className="section-title"><span>{simulated ? "Simulated lifecycle" : "Authoritative lifecycle"}</span><small>{simulated ? "disposable demo read model · SIMULATED" : "committed application state"}</small></div><div className="lifecycle-flow">{FLOW.map((step, i) => { const done = reached.has(step) || (!exceptional && currentIndex >= 0 && i < currentIndex); return <div key={step} className={`${done ? "done" : ""} ${phase === step ? "current" : ""}`}><i>{done ? "✓" : i + 1}</i><span>{FLOW_LABELS[step]}</span></div>; })}{exceptional && <div className="exception current"><i>!</i><span>{phase.replaceAll("_", " ")}</span></div>}</div></section>;
}
...
function DiagnosisCard({ incident, view }) {
  ...
  return <Card title="Evidence → diagnosis" icon="◎" meta={diagnosis ? diagnosis.provenance === "SIMULATED" ? "simulated validation" : "application validated" : "investigation active"}><div className="signal-box"><div><small>Predictive signal</small><b>{incident.predicted_mode_label || "Elevated failure risk"}</b></div><Status value="signal">prediction, not cause</Status></div>
    ...
    {diagnosis ? <><div className="diagnosis"><span className="validated-mark">✓</span><div><small>Validated diagnosis</small><h3>{diagnosis.conclusion}</h3><p>{diagnosis.evidence_ids.length} cited evidence records · confidence {diagnosis.confidence == null ? "not asserted" : pct(diagnosis.confidence)}</p></div></div>{diagnosis.affected_component && <div className="plan-grid"><KV label="Affected component" value={diagnosis.affected_component} /><KV label="Validation" value={diagnosis.validation_status} /></div>}{diagnosis.key_observations?.length > 0 && <div className="parts"><small>Supporting observations</small>{diagnosis.key_observations.slice(0, 3).map((item) => <span key={item}>{item}</span>)}</div>}</> : <EmptyLine text="No diagnosis has crossed the application validation boundary." />}{verdict && <Verdict item={verdict} />}</Card>;
}
...
function InterventionCard({ incident, view }) {
  ...
  return <Card title="Proposed intervention" icon="⌁" meta={item?.status || "not proposed"}>{item ? <><div className="plan-head"><div><small>Typed maintenance action</small><h3>{item.steps[0]?.parameters?.description || item.steps[0]?.capability?.replaceAll("_", " ")}</h3></div><Status value={item.risk}>{item.risk} risk</Status></div><div className="plan-grid"><KV label="Priority" value={item.priority} /><KV label="Component" value={item.component || item.steps[0]?.capability} /><KV label="Technician" value={binding?.technician_id} /><KV label="Required skill" value={item.required_skill || binding?.qualification} /><KV label="Estimated cost" value={money0(item.estimated_cost)} /><KV label="Downtime" value={`${item.estimated_downtime_minutes} min`} /><KV label="Window" value={formatWindow(item.window_start, item.window_end)} /><KV label="Avoided loss" value={money0(item.estimated_avoided_loss)} /></div>{binding?.parts?.length > 0 && <div className="parts"><small>Reserved parts</small>{binding.parts.map((part) => <span key={part.part_id}>{part.quantity}× {part.part_id} · {part.status || "available"}</span>)}</div>}{item.safety_note && <p className="run-summary">Safety: {item.safety_note}</p>}{verdict && <Verdict item={verdict} />}</> : <EmptyLine text={incident.lifecycle?.phase === "DIAGNOSIS_VALIDATED" ? "Diagnosis accepted. Maintenance planning is next." : "No intervention has been promoted."} />}</Card>;
}
...
function Verdict({ item }) { return <div className={`verdict ${tone(item.decision)}`}><b>{item.decision === "ACCEPT" ? item.provenance === "SIMULATED" ? "✓ SIMULATED critic + validation shown" : "✓ Critic + application validation passed" : item.decision}</b>{(item.blocking_issues || []).length > 0 && <p>{item.blocking_issues.join(" · ")}</p>}<small>{item.validation_policy_version || "validation policy"}</small></div>; }
